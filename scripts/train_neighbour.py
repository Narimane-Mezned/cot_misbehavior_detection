...
class FeatureSubsetDataset(Dataset):
    def __init__(self, base_dataset, keep_indices):
        ...
        self.keep = torch.tensor(keep_indices, dtype=torch.long)

    def __len__(self):
        return len(self.base_dataset)

    def __getitem__(self, idx):
        return self.base_dataset[idx].index_select(-1, self.keep)
...
def next_step_targets(batch: torch.Tensor):
    return batch[:, :-1, :], batch[:, 1:, :]


def run_epoch(model, loader, optimizer, delta, device, train):
...
def main():
    config_path = REPO_ROOT / "configs" / "pampos_neighbour.yaml"
    with open(config_path, "r") as f:
        config = yaml.safe_load(f)

    keep = config["features"]["keep_indices"]
    names = config["features"]["names"]

    set_seed(config["training"]["seed"])
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"[setup] Using device: {device}")
    print(f"[setup] Neighbour-context feature set: {names}")
    print(f"[setup] Kinematic features plus neighbour context")
    ...
    train_raw, val_raw = random_split(full_dataset, [train_size, val_size], generator=generator)
    print(f"[setup] Train windows: {len(train_raw)}, Val windows: {len(val_raw)}")
    print(f"[setup] Split seed matches train_baseline.py, so the two models see identical windows")

    train_sub = FeatureSubsetDataset(train_raw, keep)
    val_sub = FeatureSubsetDataset(val_raw, keep)

    processed_dir = REPO_ROOT / "data" / "processed"
    processed_dir.mkdir(parents=True, exist_ok=True)
    stats_path = processed_dir / "feature_stats_neighbour.npz"
    ...
    np.savez(stats_path, mean=mean.numpy(), std=std.numpy(), keep_indices=np.array(keep))
    ...
    train_dataset = NormalizedSequenceDataset(train_sub, mean, std)
    val_dataset = NormalizedSequenceDataset(val_sub, mean, std)
    ...
    checkpoint_path = checkpoint_dir / "pampos_neighbour_best.pt"
    ...
    print(f"[done] Compare against pampos_ablated_best.pt, which saw the same windows")
    print(f"[done] without neighbour context.")
